...
if [ $entry_name = "HCC_US_IL_BLUE_WATERS_LIGO" ]; then
  ...
  echo "Running at Blue waters"
  echo "Release set to: " $release 
  echo "gfal not checked because it is blue waters"
  exit 0
else
  release=$(rpm -E %rhel)
fi
...
case $release in
     6)
        CVMFS_WN_PATH=/cvmfs/oasis.opensciencegrid.org/osg-software/osg-wn-client/3.4/current/el6-x86_64
        ...
        ;;
     7)
        CVMFS_WN_PATH=/cvmfs/oasis.opensciencegrid.org/osg-software/osg-wn-client/3.4/current/el7-x86_64
...
export LD_LIBRARY_PATH=$CVMFS_WN_PATH/usr/lib64/:$CVMFS_WN_PATH/lib64/:$LD_LIBRARY_PATH
